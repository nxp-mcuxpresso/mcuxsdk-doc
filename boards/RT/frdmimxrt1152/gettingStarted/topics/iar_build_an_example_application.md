...
2.  Select the desired build target from the drop-down menu.
    ...
    -   `hyperram_txt_debug`– Project configuration is same as the debug target. The linker file is `hyperram_txt`linker, where text is put in hyperram and data put in OCRAM.
    -   `hyperram_txt_release`– Project configuration is same as the release target. The linker file is `hyperram_txt`linker, where text is put in hyperram and data put in OCRAM.
    ...
    For some examples need large data memory, only `hyperram_debug`and `hyperram_release`targets are supported. For this example, select **hello\_world** – **debug**.

    ![](../images/iar_demo_build_target_selection.png "Demo build target selection")

3.  To build the demo application, click **Make**, highlighted in red in [Figure 2](#frdmimxrt1152_FIG_BUILDTHEDEMOAPP).

    (frdmimxrt1152_FIG_BUILDTHEDEMOAPP)=
