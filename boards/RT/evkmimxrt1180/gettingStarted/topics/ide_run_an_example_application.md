...
4.  On the **Quickstart Panel**, click **Debug**.
    ...
    **Note:** Erase the flash memory before running the MCUX IDE projects for the first time. To do so, use the **Erase Flash action using LinkServer** in [Figure 2](ide_run_an_example_application.md#FIG_DEBUGHELLOWORLDCASES) with `SW5[1..4]` on `0100`. In the worst case when MCUX IDE cannot erase the flash, use the secure provisioning tool. For more information, see [Use Secure Provisiong Tool to erase flash](use_secure_provisiong_tool_to_erase_flash.md).
