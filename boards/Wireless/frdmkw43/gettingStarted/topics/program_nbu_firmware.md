...
*middleware\\wireless\\ble\_controller\\bin\\kw43\_nbu\_ble\_hosted.sb4*

To program the NBU software for the KW43, perform the following steps:
...
4. Erase the NBU flash region and program the NBU binary:
    ...
    **When the chip is blank (no valid NBU image at flash offset 0x0):**
    ...
    ```
    blhost.exe -p COM3 write-memory 0x80000 kw43_nbu_ble_hosted_test.sb4
    ```
    ...
    **Step 1 — Write the new NBU binary to `NBU_UPDATE_IMG_ADDR`:**
    ...
    blhost.exe -p COM3 write-memory <NBU_UPDATE_IMG_ADDR> kw43_nbu_ble_hosted_test.sb4
    ```
