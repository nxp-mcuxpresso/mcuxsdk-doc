...
1.  Get the boot images and the imx-mkimage source repository from corresponding Linux BSP release. The boot images required to be put into imx-mkimage/i.MX95 are:
    ...
    `- lpddr5_dmem_qb_v202311.bin (lpddr4x_dmem_qb_v202311.bin for FRDM-IMX95)`

    `- lpddr5_dmem_v202311.bin (lpddr4x_dmem_v202311.bin for FRDM-IMX95)`

    `- lpddr5_imem_qb_v202311.bin (lpddr4x_imem_qb_v202311.bin for FRDM-IMX95)`

    `- lpddr5_imem_v202311.bin (lpddr4x_imem_v202311.bin for FRDM-IMX95)`
...
3.  make image for ram target.
    ...
    2.  Make image for ddr target:
        ...
        `make SOC=iMX95 OEI=YES flash_m7_ddr LPDDR_TYPE=lpddr5`

        `make SOC=iMX95 OEI=YES flash_lpboot_sm_m7 LPDDR_TYPE=lpddr5`

    **Note:** For FRDM-IMX95, LPDDR\_TYPE=lpddr4x.
...
4.  Burn flash.bin to MicroSD/eMMC at 32 K\(0x8000\) offset with dd or HxD or UUU and then plug the MicroSD card to the board.
    ...
    -   Burn flash.bin to SD/eMMC with UUU
        ...
        **Note:**
        ...
        -   `imx-boot-imx95-19x19-lpddr5-evk-sd.bin-flash_all (imx-boot-imx95-15x15-lpddr4x-frdm-sd.bin-flash_all for FRDM-IMX95)`. Get it from linux bsp.
