...
### 1. Introduction
This section explains how to use MCUXpresso Cmake extensions. How to add/remove and customize projects with NXP CMake format. This guide uses the `hello_world` demo application to demonstrate how use the cmake project format.
...
The MCUXpresso SDK extends standard CMake with a few helper macros so you can focus on your application rather than wiring up every driver by hand. At a high level:
...
**mcux_add_component()** — Declares a dependency on an SDK component. For example a driver or middleware component. In most SDK examples, components are enabled via configuration (Kconfig/prj.conf) and then pulled in automatically; this macro is available when you want to request a component explicitly from CMake.
...
- Steps:
    - Create a new source file. Easiest way to do this is from the **VS Code Explorer** view.
      - Click on the **Go To Explorer** icon on the project. This will open your project's files in the primary side bar.  
      ...
      - Name the new file ***my_module.c***
        ...
        - Insert the following code snippet into ***my_module.c***
          ...
          ```
          > Notice that ***fsl_debug_console.h*** is referenced. This a component in the SDK that does not need to be explicitly added to the CMakeLists.txt file. For other components like middleware or drivers, reference the [Kconfig User Guide](using_kconfig.md) to learn how to integrate those.
    ...
    - Now we can modify the CMakeLists.txt file to include these files in the build.
      ...
      - Now, implement the functionality in the project. Open the ***hello_world.c*** file and make the changes as seen below.
        - Add
          ```
          #include "my_module.h"
          ```
...
Adding components to a project can be done with kconfig. In this section of the CMake project user guide we cover how components are exposed to CMake. Refer to the [Kconfig User Guide](using_kconfig.md) for more examples on working with Kconfig.
...
```
 This is the assembly point for all CMake data for devices, board, drivers, components, and middleware. That snippet makes all the components of the SDK available, but does not automatically compile every driver or middleware. The components that will actually be active in the project depends on Kconfig settings provided by the device and board ports on any given example in the sdk or those defined explicitly by the user.

 To highlight the Kconfig interaction with CMake, we will add the ***blinky*** functionality into the Hello_World example in the previous steps. The GPIO driver, clocks, peripherals are already enabled by default in the board port for ***Hello_world***. We will keep these configurations and only reference the GPIO driver in the source code.
  ...
  - Steps:
    ...
    - Add the new sources to the projects CMakeLists.txt file. We have provided an annotated version of this project's CMakeLists.txt file that shoes how CMake and Kconfig interact:
      >NOTE: Annotation was added to the default CMakeList.txt file to help clarify the role of each section. The annotation is only provided as part of this guide.
      ```
      # Copyright 2025 NXP
      #
      ...
      ```
  - Build and Debug the modified project. The ***Hello World*** that we started with should now print the standard hello world message, the message from the custom module that was created, and use the gpio driver to blink the on board LED.
