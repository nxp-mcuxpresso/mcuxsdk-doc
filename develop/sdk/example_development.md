...
In some cases, developers may want to export board‑specific files such as pin_mux.c/.h, clock_config.c/.h, and others. To support this, the export_app extension provides the optional `--bf` argument, which allows developers to collect all board‑related files guarded by `CONFIG_MCUX_PRJSEG_xxx`.
The `--bf` argument accepts zero or multiple values. Additionally, developers can specify default search directories through the `freestanding_copied_folders` field in `example.yml`.

```yaml
hello_world:
  section-type: application
  contents:
    # value here should be a relative path to mcuxsdk/
    freestanding_copied_folders:
    - my_example1
    - my_example2
```

So you can use following combinations:

- `--bf` with no value: copy all board‑related files under the default example directory `mcuxsdk/examples`.
- `--bf <folder1> <folder2> ...`: copy board‑related files from the given folders.
- `freestanding_copied_folders`: copy board‑related files from directories specified in `example.yml`.

```{note}
For `freestanding_copied_folders` to take effect, the `--bf` argument must be provided. The final search directories will then be the combination of the folders listed in `freestanding_copied_folders` in `example.yml` and the directories specified by `--bf`. You may pass `--bf` without any values if you want to use only the directories defined in `freestanding_copied_folders`.
```

With `--bf`, you can get a Freestanding project like this:
