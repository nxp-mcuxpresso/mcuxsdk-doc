...
class VersionFilter(PageFilter):
    ...
    def run(self):
        ...
        versions = list(map(create_tuple, reversed(self.env.sdk_versions)))
...
def read_versions(app: Sphinx) -> None:
    """Get all SDK versions to date"""

    if hasattr(app.env, "sdk_versions") and app.env.sdk_versions:
        return

    try:
        with open(VERSIONS_FILE) as version_file:
            sdk_versions = json.loads(version_file.read())
            sdk_versions = list(
                filter(lambda v: re.match(r"\d+\.\d+\.[\d\w-]+$", v), sdk_versions)
            )
            # Versions classes are on the format "vX-X-X"
            app.env.sdk_versions = [
                f"v{version.replace('.', '-')}" for version in reversed(sdk_versions)
            ]
