.. _mbedtls3x_3rd_party_architecture:

Architecture
############

.. toctree::
   :maxdepth: 1
   :glob:

   ../docs/architecture/alternative-implementations.md
   ../docs/architecture/mbed-crypto-storage-specification.md
   ../docs/architecture/psa-crypto-implementation-structure.md
   ../docs/architecture/psa-keystore-design.md
   ../docs/architecture/psa-migration/md-cipher-dispatch.md
   ../docs/architecture/psa-migration/psa-legacy-bridges.md
   ../docs/architecture/psa-migration/psa-limitations.md
   ../docs/architecture/psa-migration/strategy.md
   ../docs/architecture/psa-migration/testing.md
   ../docs/architecture/psa-migration/transition-guards.md
   ../docs/architecture/psa-shared-memory.md
   ../docs/architecture/psa-storage-resilience.md
   ../docs/architecture/psa-thread-safety/psa-thread-safety.md
   ../docs/architecture/testing/driver-interface-test-strategy.md
   ../docs/architecture/testing/invasive-testing.md
   ../docs/architecture/testing/psa-storage-format-testing.md
   ../docs/architecture/testing/test-framework.md
   ../docs/architecture/tls13-support.md